...
async function main() {
  ...
  const emojiText = replaceEmoji('Hello :wave: World! :rocket: Ready to :code: with :heart:');
  console.print(emojiText);

  console.print('\n[bold]Available emoji shortcodes:[/]');
  const sampleEmojis = ['rocket', 'star', 'fire', 'heart', 'check', 'warning', 'coffee', 'party'];
  ...
  console.print(`[dim]Total spinners available: ${listSpinners().length}[/dim]`);
  ...
  summaryTable.addRow(replaceEmoji(':sparkles: Emoji'), '200+ shortcodes like :rocket: :heart:');
  summaryTable.addRow(replaceEmoji(':package: JSON'), 'Pretty-printed, syntax-highlighted JSON');
  summaryTable.addRow(replaceEmoji(':art: Columns'), 'Responsive multi-column layouts');
  summaryTable.addRow(replaceEmoji(':gear: Spinner'), '40+ animated spinner styles');
  summaryTable.addRow(replaceEmoji(':zap: Live'), 'Real-time display updates');
  summaryTable.addRow(replaceEmoji(':bulb: Status'), 'Animated status indicators');
