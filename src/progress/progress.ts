...
export class Progress {
	...
	addTask(
		description: string,
		options: { total?: number; completed?: number } = {},
	): number {
	...
	update(
		taskId: number,
		options: { completed?: number; description?: string },
	): void {
	...
	private refresh(): void {
		...
		for (const task of this.tasks.filter((t) => t.visible)) {
			...
			const percentStyle =
				percent >= 100 ? Style.parse("#50fa7b bold") : Style.parse("#61afef");
			lines.push(
	...
	private renderSimpleBar(
		...
		const filledStyle =
			percentage >= 1 ? Style.parse("#50fa7b bold") : Style.parse("#61afef");
